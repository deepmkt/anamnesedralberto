...
/**
 * Salva o lead. Caminho principal: webhook first-party /api/lead
 * (bloqueadores não barram; keepalive sobrevive à navegação).
 * Fallback: POST direto no Supabase, se configurado no client.
...
export async function saveLead(payload) {
  ...
  try {
    ...
    if (res.ok) return;
    console.error("saveLead: webhook falhou", res.status);
  } catch (e) {
    console.error("saveLead webhook:", e);
  }

  // Fallback direto (só roda se o webhook falhar)
  ...
  try {
    ...
    console.error("saveLead fallback:", e);
  }
}
